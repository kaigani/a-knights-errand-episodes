...
async function preloadAll(onProgress) {
  // v4-labyrinth: 61 passage ids share only 27 distinct clip files (a
  // corridor clip is played from many passages). Fetch each unique `src`
  // ONCE, then fan the resulting blob URL out to every passage id that
  // shares it -- BUILD_CONTRACT Sec4's only allowed main.js change. Every
  // other function still reads `blobs[id]` by passage id unchanged.
  const ids = Object.keys(manifest.clips);
  const idsBySrc = new Map();
  for (const id of ids) {
    const src = manifest.clips[id].src;
    if (!idsBySrc.has(src)) idsBySrc.set(src, []);
    idsBySrc.get(src).push(id);
  }
  const uniqueSrcs = [...idsBySrc.keys()];
  let done = 0;
  await Promise.all(uniqueSrcs.map(async (src) => {
    const r = await fetch(src);
    if (!r.ok) throw new Error(`fetch ${src}: ${r.status}`);
    const blobUrl = URL.createObjectURL(await r.blob());
    for (const id of idsBySrc.get(src)) blobs[id] = blobUrl;
    onProgress(++done / uniqueSrcs.length);
  }));
}
...
function showEnd(outcome) {
  ...
  logo.alt = outcome === "win" ? "The Chalice Is Won!" : "The Labyrinth Keeps Its Own";
  const names = path.map((id) => manifest.clips[id].title || id);
  $("endPath").innerHTML =
    `Your path: ${names.join(" → ")}` +
    (outcome === "win" ? "<br>Out into the Daylight, Chalice in Hand!" : "<br>The Labyrinth Keeps You. Try once more.");
  restartBtn.focus();
}
