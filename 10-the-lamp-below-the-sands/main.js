 ...
 * the primed incoming <video> at either the selected choice's next loop
 * boundary or the natural end of a non-choice clip, then reveal it on its
 * first actually-rendered frame (requestVideoFrameCallback).
...
let swapping = false;
let choice = null;     // {info, phase: pending|open|locked, picked, branchAt}
let swapT0 = 0;
...
function beginClipState(id) {
  ...
  choice = info.choice
    ...
    : null;
  hideChoiceUI();
  // beat nodes (no choice, just a `next`) auto-advance: arm the next clip
  // right away, same as a locked choice, so the tick loop's normal
  // armed-swap-near-end path (see logicStep/doSwap) carries it across
  // exactly like a picked option, with no QTE ever shown.
  if (!choice && info.next) armNext(info.next);
...
function nextLoopBoundary(v, info) {
  // The choice window is an integer number of mechanism cycles appended to
  // the transition.  Once the player commits, finish only the cycle currently
  // on screen; the selected clip begins at the following cycle's pre-roll.
  const loopStart = Math.max(0, v.duration - info.window);
  const elapsed = Math.max(0, v.currentTime - loopStart);
  const cycle = Number.isFinite(info.cycle) ? info.cycle : LOOP_CYCLE;
  const nextCycle = Math.floor((elapsed + 0.001) / cycle) + 1;
  return Math.min(v.duration, loopStart + nextCycle * cycle);
}
...
function pick(i, auto = false) {
  ...
  choice.phase = "locked";
  choice.picked = i;
  choice.branchAt = nextLoopBoundary(active, choice.info);
  ...
  if (DEBUG) console.log(
    `[choice] ${currentId}: ${auto ? "auto" : "input"} -> ${next} at ${choice.branchAt.toFixed(3)}`
  );
  armNext(next);
}
...
function logicStep() {
  ...
  if (armed && !swapping) {
    // A picked choice branches at the next 4.25 s loop boundary. Automatic
    // beat nodes retain the end-of-clip lead used by the latent-chain player.
    const pickedBoundary = choice?.phase === "locked" ? choice.branchAt : null;
    const branchDue = Number.isFinite(pickedBoundary)
      ? v.currentTime >= pickedBoundary
      : v.duration - v.currentTime <= SWAP_LEAD;
    if (branchDue) doSwap();
  }
}
...
function tick() {
  ...
  if (DEBUG && mode === "playing" && active.duration) {
    hud.textContent =
      ...
      `armed  ${armed || "-"}\n` +
      `branch ${Number.isFinite(choice?.branchAt) ? choice.branchAt.toFixed(2) : "-"}\n` +
      `path   ${path.join(" > ")}`;
  }
...
function showEnd(outcome) {
  ...
  logo.alt = outcome === "win" ? "The Lamp is Stoppered!" : "Granted…";
  const names = path.map((id) => manifest.clips[id].title || id);
  $("endPath").innerHTML =
    `Your path: ${names.join(" → ")}` +
    (outcome === "win" ? "<br>The djinn is in his lamp and the desert is one dune lighter!" : "<br>Granted exactly as asked. Try once more.");
  restartBtn.focus();
}
